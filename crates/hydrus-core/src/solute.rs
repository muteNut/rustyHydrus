...
impl Simulation {
    ...
    pub fn sol_wlayer_mix(&mut self) {
        if !(self.w_layer && self.h_new[self.n - 1] > 0.0) {
            return;
        }
        let (ht, dt, prec, r_soil) = (self.h_new[self.n - 1], self.dt, self.prec, self.r_soil);
        if let Some(s) = self.sol.as_mut() {
            for j in 0..s.ns {
                let den = ht + dt * (prec - r_soil);
                if den > 0.0 {
                    s.c_top[j] = (ht * s.c_top[j] + dt * prec * s.c_t[j]) / den;
                }
            }
        }
    }
    ...
    pub fn solute_init(&mut self) -> Result<(), HydrusError> {
        ...
        for m in 0..n_mat {
            ...
            if mm.frac < 1.0 || mm.th_immobile > 0.0 {
                l_equil = false;
            }
                ...
                for i in 0..n {
                    if conc[j][i] > 0.0 {
                        ...
                        conc[j][i] = if l_linear[j] {
                            mass / (th + ro[m] * p.ks + tha * p.henry)
                        } else {
                            let f = |c: f64| th * c + ro[m] * p.ks * c.powf(p.beta) / (1.0 + p.nu * c.powf(p.beta)) + tha * p.henry * c;
    ...
    fn solute_inner(&mut self, st: &mut SoluteState) -> Result<(), HydrusError> {
        ...
        'restart: loop {
            ...
            for js in 0..ns {
                ...
                if self.sink_f {
                    for i in 0..n {
                        st.s_sink[i] = if self.beta[i] > 0.0 { self.sink[i] * st.conc[js][i].min(st.c_root_max[js]).max(0.0) } else { 0.0 };
                    }
                } else {
    ...
    fn sol_coeff(&self, st: &mut SoluteState, js: usize, level: usize, iter: usize, dt: f64) -> (f64, f64, f64) {
        ...
        for i in (0..n).rev() {
            ...
            let mut disp = st.disp_l[m] * v.abs() / th_w + st.diff_w[js] * tau_w + th_g / th_w * st.diff_g[js] * henry * tau_g;
            if !st.art_d && !st.upw {
                let mut fi = 0.0;
                if c_mid > 0.0 {
            ...
            if !st.l_equil {
                if last {
                    if mob {
                        ...
                        let e_mob = th_imob * xmu_l + (1.0 - frac) * ro * xmu_s;
                        let b_mob = th_imob * (gam_l + gam_l1) + (1.0 - frac) * ro * (gam_s + gam_s1) * xks * sconc_s;
                        if st.l_linear[js] {
                            let d_mob = 2.0 * a_mob + dt * (omega + b_mob);
                            st.d_mob_i = d_mob;
                            let g_mob0 = (2.0 * a_mob - dt * (omega + b_mob)) / d_mob;
                            st.sorb[js][i] = st.sorb[js][i] * g_mob0 + dt * (omega * st.conc[js][i] + 2.0 * e_mob) / d_mob;
                            ssorb = st.sorb[js][i];
                        } else {
                            let cc = st.c_new[i];
                            ...
                            st.sorb_n[i] = s_old
                                + dt / a_mob
                                    * (epsi * (omega * (cc - st.sorb_n[i]) - b_mob * st.sorb_n[i] + e_mob)
                                        + (1.0 - epsi) * (omega * (st.conc[js][i] - s_old) - b_mob * s_old + e_mob));
                            ssorb = st.sorb_n[i];
                        }
                        ...
                        let cc = st.c_new[i];
                        st.sorb_n[i] = s_old
                            + dt * (epsi * (omega * ((1.0 - frac) * sconc * xks * cc - st.sorb_n[i]) - (gam_s + gam_s1) * st.sorb_n[i] + (1.0 - frac) * xmu_s)
                                + (1.0 - epsi)
                                    * (omega * ((1.0 - frac) * sconc_o * xks * st.conc[js][i] - s_old) - (gam_s + gam_s1) * s_old + (1.0 - frac) * xmu_s));
                        ssorb = st.sorb_n[i];
                    }
                }
            }
            ...
            if let Some(pp) = &parent {
                let cprev = if last { st.conc[js - 1][i] } else { st.c_prev_o[i] };
                let cg = cprev * (pp.gam_w * th_w + ro * frac * pp.ks * pp.gam_s * sconc_p + th_g * pp.henry * pp.gam_g);
                let mut cg1 = cg;
                if !st.l_equil {
                    if mob {
                        cg1 += st.sorb[js - 1][i] * (th_imob * pp.gam_w + (1.0 - frac) * ro * pp.gam_s * pp.ks * sconc_ps);
                    } else {
                        cg1 += pp.gam_s * ro * st.sorb[js - 1][i];
                    }
                }
                g0 += cg;
                q0 += cg1;
            ...
            st.g1[i] = g1;
            let mut q1 = (-(gam_l + gam_l1) * th_w - (gam_s + gam_s1) * ro * frac * xks * sconc - (gam_g + gam_g1) * th_g * henry) * st.conc[js][i];
            if !st.l_equil {
                if mob {
                    q1 -= st.sorb[js][i] * (th_imob * (gam_l + gam_l1) + (1.0 - frac) * ro * xks * sconc_s * (gam_s + gam_s1));
                } else {
                    q1 -= (gam_s + gam_s1) * ro * st.sorb[js][i];
                }
            }
            st.q1[i] = q1;
            // ---- velocity correction (gas-phase diffusion, varying Henry constant)
            let hen = |mm: usize| st.pm[js][mm].henry;
            let der_k;
            if i == 0 {
                der_k = (hen(self.mat[j]) - henry) / (self.x[1] - self.x[0]);
            } else if i == n - 1 {
                der_k = (henry - hen(self.mat[i - 1])) / (self.x[n - 1] - self.x[n - 2]);
            } else {
                der_k = (hen(self.mat[j]) - hen(self.mat[i - 1])) / ((self.x[j] - self.x[i - 1]) / 2.0);
            }
            let vcorr = th_g * st.diff_g[js] * tau_g * der_k;
            st.v_corr[i] = vcorr;
            if level == 1 {
                st.v_o[i] -= vcorr;
            } else {
    ...
    fn sol_sorbconc(&self, st: &mut SoluteState, js: usize, dt: f64) {
        for i in 0..self.n {
            ...
            if st.l_mob_im[m] {
                ...
                let b_mob = th_im * (p.mu_w + p.gam_w) + (gam_s + gam_s1) * ro * (1.0 - frac) * xks;
                let d_mob = 2.0 * a_mob + dt * (omega + b_mob);
                st.sorb[js][i] += dt * omega * st.conc[js][i] / d_mob;
            } else {
    ...
    pub fn sub_reg_solute(&mut self, p_level: usize, rec: &mut BalanceOut) {
        ...
        for js in 0..st.ns {
            ...
            for i in (0..n - 1).rev() {
                ...
                let dx = self.x[j] - self.x[i];
                let (pi_, pj) = (&st.pm[js][mi], &st.pm[js][mj]);
                let side = |m: usize, p: &SpeciesMaterial, th: f64, cc: f64| {
                    let mut thw = th;
                    let thg = (self.ths[m] - thw).max(0.0);
                    if st.l_mob_im[m] {
                        thw = (thw - st.th_im[m]).max(0.001);
                    }
                    let c1 = if !st.l_linear[js] && cc > 0.0 { cc.powf(p.beta - 1.0) / (1.0 + p.nu * cc.powf(p.beta)) } else { 1.0 };
                    cc * (thw + st.frac[m] * st.ro[m] * p.ks * c1 + thg * p.henry)
                };
                let cnew = dx / 2.0 * (side(mi, pi_, self.th_new[i], st.conc[js][i]) + side(mj, pj, self.th_new[j], st.conc[js][j]));
                vol += cnew;
                sub_v[lay] += cnew;
                if !st.l_equil {
                    let sdside = |m: usize, p: &SpeciesMaterial, s: f64| {
                        if st.l_mob_im[m] {
                            let s1 = if !st.l_linear[js] && s > 0.0 { s.powf(p.beta - 1.0) / (1.0 + p.nu * s.powf(p.beta)) } else { 1.0 };
                            s * (st.th_im[m] + (1.0 - st.frac[m]) * st.ro[m] * p.ks * s1)
                        } else {
                            st.ro[m] * s
                        }
                    };
                    vol_im += dx / 2.0 * (sdside(mi, pi_, st.sorb[js][i]) + sdside(mj, pj, st.sorb[js][j]));
                }
                ...
                if js == 0 || true {
                    if p_level == 0 {
                        ...
                        delt_c += (st.sol_in[js][i] - cnew).abs();
                    }
                }
            }
            for l in 0..n_lay {
